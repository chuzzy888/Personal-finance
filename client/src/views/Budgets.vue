...
<template>
  <div class="max-w-6xl mx-auto md:p-6">
    <div v-if="showToast" class="toast josefin-font">
      {{ toastMessage }}
    </div>
    ...
    <transition name="fade">
      <div
        ...
        class="fixed inset-0 bg-black bg-opacity-70 z-50 josefin-font flex items-center justify-center"
      >
        <div
    ...
    >
      ...
      >
        ...
        <div class="flex items-center space-x-4">
          ...
          <button
            @click="openConfirmationModal(budget._id)"
            class="text-red-600 hover:text-red-700 px-3 py-1 rounded-lg border border-red-600 hover:bg-red-50 transition duration-200"
          >
            Delete
          </button>
  ...
  methods: {
    ...
    addBudget() {
      axiosInstance
        ...
        .then(() => {
          this.fetchBudgets();
          this.closeModal();
          this.showToastMessage("Budget added successfully!");
        })
        .catch(error => {
          console.error("Error adding budget:", error);
        });
    ...
    updateBudget() {
      axiosInstance
        ...
        .then(() => {
          this.fetchBudgets();
          this.closeModal();
          this.showToastMessage("Budget updated successfully!");
        })
        .catch(error => {
          console.error("Error updating budget:", error);
        });
    ...
    confirmDelete() {
      if (this.budgetToDelete) {
        this.deleteBudget(this.budgetToDelete);
      }
